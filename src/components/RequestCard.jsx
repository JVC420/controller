...
export const RequestCardUI = ({ request, client, isDragging, style, attributes, listeners, setNodeRef, onEdit, compact = false }) => {
    ...
    const cardClasses = clsx(
        "relative bg-dark-800 rounded-xl transition-all w-full flex flex-col group select-none",
    ...
    return (
        <div ref={setNodeRef} style={style} className={cardClasses} {...attributes} {...listeners}>
            ...
            <div className="flex justify-between items-start">
                ...
                <div className={clsx("flex items-center gap-1 font-bold px-2 py-1 rounded-md",
            ...
            {client && (
                <div className="flex flex-col gap-1">
                    ...
                    <p className={clsx("text-slate-400", compact ? "text-[11px]" : "text-xs")}>{client.tipo}</p>
                </div>
            )}

            <div className={clsx("rounded-lg border border-slate-800 bg-dark-900/40", compact ? "px-2.5 py-1.5" : "px-3 py-2")}>
                <span className="text-[10px] uppercase font-bold text-slate-500">Paciente</span>
                <p className={clsx("text-slate-100 font-medium leading-tight mt-1", compact ? "text-xs" : "text-sm")}>{patientName}</p>
            </div>

            {/* Locations */}
            <div className={clsx("space-y-2 bg-dark-900/50 rounded-lg border border-slate-800", compact ? "mt-1 p-2.5" : "mt-2 p-3")}>
                <div className="flex items-start gap-2">
                    <MapPin size={14} className="text-emerald-500 mt-0.5 shrink-0" />
                    <div className="flex flex-col">
                        <span className="text-[10px] uppercase font-bold text-slate-500">Origen</span>
                        <span className={clsx("text-slate-200 leading-tight", compact ? "text-xs" : "text-sm")}>{request.origen}</span>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-2">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className={clsx("text-slate-200 leading-tight", compact ? "text-xs" : "text-sm")}>{request.destino}</span>
                    </div>
                </div>
            </div>
        </div>
